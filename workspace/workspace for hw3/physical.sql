...
-- This table represents the quantity of a specific product stored in a store, its re-order level and the order quantity
CREATE TABLE Stores (
	productid SERIAL,
	storeid SERIAL,
...
COMMENT ON TABLE Stores IS 'Represents the quantity of a specific product stored in a store, its re-order level and the order quantity.';
...
CREATE TABLE Promotion (
	productid SERIAL,
	storeid SERIAL,
	start_enddates DATERANGE NOT NULL,
...
COMMENT ON COLUMN Promotion.start_enddates IS 'Initial and final date of a promotion.';
...
COMMENT ON COLUMN Supplier.contractdetails IS 'Details and description of the contract between store and supplier.';
...
COMMENT ON TABLE Supplies IS 'Represents the quantity of a specific product supply by a supplier to a store.';
...
-- This table represents a employee
...
COMMENT ON TABLE Employee IS 'Represents a employee.';
...
COMMENT ON COLUMN Receipt.employeeid IS 'Unique identifier for the employee who attended during the purchase.';
...
CREATE TABLE BelongsTo (
	...
	PRIMARY KEY ( receiptid, email )
);

COMMENT ON TABLE BelongsTo IS 'Associates the customer with the receipt (if customer used the loyalty program while purchasing).';
